...
public class ConsoleLogger {
    ...
    private void sendLog(ConsoleLogMessage consoleLogMessage) {
        DefaultGoApiRequest request = new DefaultGoApiRequest(Constants.SEND_CONSOLE_LOG, Constants.CONSOLE_LOG_PROCESSOR_API_VERSION, Constants.PLUGIN_IDENTIFIER);
        ...
        if (response.responseCode() != DefaultGoApiResponse.SUCCESS_RESPONSE_CODE) {
            DockerRegistryArtifactPlugin.LOG.error(String.format("Failed to submit console log: %s", response.responseBody()));
        }
    }
